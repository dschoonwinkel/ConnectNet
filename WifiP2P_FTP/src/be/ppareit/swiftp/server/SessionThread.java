...
/**
 * The workhorse class of the Ftp Server. This class handles Socket IO, as well
 * as regulating the state of the Ftp Server.
 * 
 * @author David Revell, Pieter Parreit
 * 
 */
public class SessionThread extends Thread {
	...
	public int receiveFromDataSocket(byte[] buf) {
		...
		try {
			...
			if (bytesRead == -1) {
				...
				// remaining, so we return -1, i.e. the socket was
				// closed successfully after transmission.
				return -1;
			}
		} catch (IOException e) {
	...
	public void run() {
		...
		if (sendWelcomeBanner) {
			writeString("220 SwiFTP " + Defaults.getVersion() + " ready\r\n");
		}
