...
/**
 *  PORT - Overrides the defaults for data connection. Argument is <h1,h2,h3,h4,p1,p2> style
 *  
...
public class CmdPORT extends FtpCmd implements Runnable {
    ...
    public static String buildCommand(InetAddress address, int port) {
    	String cmd = "PORT ";
    	
    	if (address == null) {
    		Log.e(TAG, "Address was null, bailing out!");
    		return "";
    	}
    	
    	String address_str = address.getHostAddress();
//    	Log.d(TAG, address_str);
    	address_str = address_str.replace(".", ",");
    	cmd += address_str;
    	cmd += ",";
    	cmd += Integer.toString(port / 256);
    	cmd += ",";
    	cmd += Integer.toString(port % 256);
