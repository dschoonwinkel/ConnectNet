...
/**
 * This is the main activity for swiftp, it enables the user to start the server
 * service and allows the users to change the settings.
 * 
 */
public class FsPreferenceActivity extends PreferenceActivity implements
		...
		marketVersionPref
				.setOnPreferenceClickListener(new OnPreferenceClickListener() {
					...
					public boolean onPreferenceClick(Preference preference) {
						...
						intent.setData(Uri
								.parse("market://details?id=be.ppareit.swiftp"));
						try {
	...
	protected void onResume() {
		...
		filter.addAction(FsService.ACTION_UPDATE_FTPSERVER);
	...
	private void updateRunningState() {
		...
		if (FsService.isRunning() == true) {
			...
			// The IP will be displayed as soon as it is ready
	...
	private void updateServerIPAddress() {
		Log.v(TAG, "updateServerIPAddress");
		Resources res = getResources();
		TwoStatePreference runningPref = findPref("running_switch");
		...
		String iptext = "ftp://" + FsService.getLocalInetAddress() + ":" + FsSettings.getPortNumber()
				+ "/";
		String summary = res
	...
	BroadcastReceiver mFsActionsReceiver = new BroadcastReceiver() {
		...
		public void onReceive(Context context, Intent intent) {
			...
			// action will be ACTION_STARTED or ACTION_STOPPED, or UPDATE_STATE
			updateRunningState();

			// The update to the InetAddress is done here. So it must either be
			// ready here,
			// or updated later
			
			if (intent.getAction().equals(FsService.ACTION_UPDATE_FTPSERVER)) {
				updateServerIPAddress();
			}
			...
			if (intent.getAction().equals(FsService.ACTION_FAILEDTOSTART)) {
				...
				mHandler.postDelayed(new Runnable() {
					...
					public void run() {
						runningPref
								.setSummary(R.string.running_summary_stopped);
					}
				}, 5000);
			}
		}
	...
	static private String transformPassword(String password) {
		...
		Resources res = context.getResources();
		String showPasswordString = res
				.getString(R.string.show_password_default);
